...
object S3Util {
  ...
  def getS3FileName(sourceTopic: String, dateString: String /* yyyy/MM/dd/HH/mm */, partitionId: Int, lastOffset: Long, batchSize: Int, fileSuffix: String): String =
    sourceTopic + seperator + dateString.replaceAll(s3Delimiter, seperator) + seperator + partitionId + seperator + (lastOffset+1) + seperator + batchSize + seperator + archiveSuffix + seperator + fileSuffix
