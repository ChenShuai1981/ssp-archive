...
import com.vpon.ssp.report.archive.actor.PartitionActorProtocol.{ResumeWork, PauseWork, ResetWork}
import com.vpon.ssp.report.archive.actor.PartitionMasterProtocol.{PartitionStat, GetKafkaConnection, ReportPartitionStat}
import com.vpon.ssp.report.archive.actor.PartitionMetricsProtocol.{GetInfo, GetMetrics}
...
class WebServiceActor extends Actor with ActorLogging with ArchiveConfig {
  ...
  private[this] def index() = {
    val html =
      """
        ...
        |<html>
        |<body>
        |<h2>Welcome to ssp-archive application console</h2>
        |<ol>
  ...
  private[this] def reportKafka() = {
    val s = sender()
    val partitionMaster = context.system.actorSelection("/user/archive-partitions")
  ...
  private[this] def reportMetrics() = {
    val s = sender()
    val defaultFields: Map[String, JsValue] = Map("jvm" -> jvmMBeansStatsPresentation, "akka" -> akkaMBeansStatsPresentation)
    val partitionMaster = context.system.actorSelection("/user/archive-partitions")
    ...
    askPartitionMetricsFuture.foreach { case PartitionStat(partitions) =>
      ...
      Future.sequence{
        sortedPartitions.map(p => {
          ...
          val metrics = context.system.actorSelection(s"/user/archive-partitions/archive-${p}/archive-metrics-${p}")
  ...
  private[this] def updateLastOffset(uri:Uri) = {
    ...
    val partitionActor = context.system.actorSelection(s"/user/archive-partitions/archive-${partitionId}")
  ...
  private[this] def operateAll(cmd: Command) = {
    ...
    askFuture.foreach { case PartitionStat(partitions) =>
      ...
      sortedPartitions.foreach(p => {
        val partitionActor = context.system.actorSelection(s"/user/archive-partitions/archive-${p}")
        partitionActor ! message
      })
      s ! HttpResponse(entity = HttpEntity(`text/plain`, s"Archive service was ${action} on this node"))
    }
    askFuture.onFailure {
      case e: AskTimeoutException =>
        s ! HttpResponse(status = StatusCodes.RequestTimeout, entity = "Akka Timed Out")
  ...
  private[this] def operate(uri:Uri, cmd:Command) = {
    ...
    if (!partitionId.isEmpty) {
      ...
      val partitionActor = context.system.actorSelection(s"/user/archive-partitions/archive-${partitionId}")
      partitionActor ! message
      s ! HttpResponse(entity = HttpEntity(`text/plain`, s"Archive service was ${action} on the partition ${partitionId}"))
    } else {
      s ! HttpResponse(entity = HttpEntity(`text/plain`, s"Can not get query parameter 'pid' value"))
    }
  }
  ...
  private[this] def info(uri:Uri) = {
    ...
    if (!partitionId.isEmpty) {
      ...
      val metrics = context.system.actorSelection(s"/user/archive-partitions/archive-${partitionId}/archive-metrics-${partitionId}")
